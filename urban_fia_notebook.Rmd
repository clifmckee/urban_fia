...
author: "Clifton McKee, Meghan Avolio started editing 1/5/26"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
* Data sources:
  ...
  + [USDA Plant Hardiness Zones](https://prism.oregonstate.edu/projects/plant_hardiness_zones.php)
...
# Import EPA Ecoregions level II data
ecoregions2 <- st_read("Data/Ecoregions/na_cec_eco_l2/NA_CEC_Eco_Level2.shp",
                       quiet = TRUE) %>% 
  mutate(NA_L2NAME = str_to_lower(NA_L2NAME))

# City ecoregions
city_ecoreg <- read_csv("Data/Classifications.csv") %>% 
  clean_names() %>% 
  mutate(across(classification_1:classification_3, str_to_lower))
...
# Total city plots table
tree_city_merge %>% 
  arrange(lng) %>% 
  select(city, plot_count_Curr) %>% 
  kable()
```

#### Number of plots with live trees

```{r plots-with-trees}
# Number of plots with live trees per city
trees_coords_merge %>%
  group_by(city, lat, lng) %>%
  summarize(plots = n_distinct(PLOTID), .groups = "drop") %>% 
  ggplot() +
  geom_col(aes(x = reorder(city, lng, decreasing = FALSE),
               y = plots)) +
  labs(x = "City (ordered by longitude)", y = "Total city plots with live trees") +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Total city plots with live trees table
trees_coords_merge %>% 
  group_by(city, lat, lng) %>%
  summarize(plots = n_distinct(PLOTID), .groups = "drop") %>% 
  arrange(lng) %>% 
...
#### Proportion of total plots with live trees

```{r pct-plots-with-trees}
# Proportion of all plots with live trees per city
trees_coords_merge %>%
  group_by(city, lat, lng) %>%
  summarize(plots = n_distinct(PLOTID), .groups = "drop") %>% 
  full_join(tree_city_merge) %>%
  group_by(city, lat, lng) %>%
  summarize(proportion = plots/plot_count_Curr) %>% 
  ggplot() +
  geom_col(aes(x = reorder(city, lng, decreasing = FALSE),
               y = proportion)) +
  scale_y_continuous(name = "Proportion of all city plots with trees",
                     limits = c(0, 1), breaks = seq(0, 1, 0.2)) +
  labs(x = "City (ordered by longitude)") +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Total city plots with trees table
trees_coords_merge %>%
  group_by(city, lat, lng) %>%
  summarize(plots = n_distinct(PLOTID)) %>% 
  full_join(tree_city_merge) %>%
  group_by(city, lat, lng) %>%
  summarize(proportion = plots/plot_count_Curr, .groups = "drop") %>% 
  arrange(lng) %>% 
...
```{r land-ownership-area}
# Plot the proportion of city acreage by ownership
tree_city_merge %>% 
  pivot_longer(c(OtherFederal:StateLocal, Water),
               names_to = "Ownership",
               values_to = "Acreage") %>% 
  ggplot() +
  geom_col(aes(x = reorder(city, lng, decreasing = FALSE),
...
# City acreage by ownership table
tree_city_merge %>% 
  select(city, c(OtherFederal:StateLocal, Water)) %>% 
  kable(digits = 0)
```

#### Ownership of plots

```{r land-ownership-plots}
# Plot the proportion of city plots by ownership
city_plots_merge %>% 
  ...
  labs(x = "City (ordered by longitude)", y = "Proportion of city plots") +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# City plots by ownership table
city_plots_merge %>% 
  group_by(city, lat, lng, OWNGRPCD) %>% 
  summarize(n = n(), .groups = "drop") %>% 
  arrange(lng) %>% 
  pivot_wider(names_from = OWNGRPCD,
              values_from = n) %>% 
  select(-lat, -lng) %>% 
  kable()
```

#### Ownership of plots with live trees

```{r land-ownership-plots-trees}
# Plot the proportion of city plots with trees by ownership
trees_coords_merge %>% 
  group_by(city, lat, lng, OWNGRPCD) %>% 
  ...
  geom_col(aes(x = reorder(city, lng, decreasing = FALSE),
           ...
           position = "fill") +
  scale_fill_viridis_d(name = "Ownership", option = "H") +
  labs(x = "City (ordered by longitude)", y = "Proportion of city plots with trees") +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# City plots with trees by ownership table
trees_coords_merge %>% 
  group_by(city, lat, lng, OWNGRPCD) %>% 
...
```{r land-ownership-trees}
# Plot the proportion of city trees by ownership
fig2a<-trees_coords_merge %>% 
  group_by(city, lat, lng, OWNGRPCD) %>% 
  summarize(n = n()) %>% 
  ggplot() +
...
# Plot the proportion of city trees by ownership on forested land only
fig2b<-trees_coords_merge %>% 
  ...
  geom_col(aes(x = reorder(city, lng, decreasing = FALSE),
               ...
               fill = OWNGRPCD),
           position = "fill") +
  scale_fill_manual(name = "Ownership", values=c('midnightblue', 'aquamarine2', 'darkgoldenrod1')) +
  labs(x = "City (ordered by longitude)", y = "Proportion of city trees in Forested Areas") +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("Results/prop_trees_ownership_forestsubset.pdf", height = 5, width = 7)
...
planted_trees_table <- trees_coords_merge %>% 
...
Fig2c<-planted_trees_table %>% 
  ...
  labs(x = "City (ordered by longitude)", y = "Planted status of trees") +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom") #+
  #facet_grid(cols = vars(classification_3), scales = "free_x", space = "free_x")
ggsave("Results/planted_trees_prop.pdf", height = 5, width = 7)

fig2<-grid.arrange(fig2a, fig2b, Fig2c, nrows=3)#why is this not working

# Table
planted_trees_table %>% 
  select(city, classification_3, planted_def, n) %>% 
  kable()
write_csv(planted_trees_table, "Results/planted_trees_table.csv")
```
...
fig2b<-trees_coords_merge %>% 
  ...
  geom_col(aes(x = reorder(city, lng, decreasing = FALSE),
           ...
           position = "fill") +
  scale_fill_manual(name = "Ownership", values=c('midnightblue', 'aquamarine2', 'darkgoldenrod1')) +
  labs(x = "City (ordered by longitude)", y = "Proportion of city trees basal area in Forested Areas") +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("Results/prop_trees_ownership_forestsubset_BasalArea.pdf", height = 5, width = 7)
...
planted_trees_tableBA %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom") #+
  #facet_grid(cols = vars(classification_3), scales = "free_x", space = "free_x")
ggsave("Results/planted_trees_prop_BA.pdf", height = 5, width = 7)
```


#### Land use type of city land area

```{r land-use-area}
# Plot the proportion of city acreage by landuse type
tree_city_merge %>% 
  pivot_longer(c(Agriculture:`Multi-family`,
               OtherNonforest:Water),
               names_to = "Type",
               values_to = "Acreage") %>%
  ggplot() +
  geom_col(aes(x = reorder(city, lng, decreasing = FALSE),
...
# City acreage by landuse type table
tree_city_merge %>% 
  arrange(lng) %>% 
  select(city,
         c(Agriculture:`Multi-family`,
           OtherNonforest:Water)) %>% 
  kable(digits = 0)
```

#### Land use type of plots

```{r land-use-plots}
# Plot the proportion of city plots by landuse type
city_plots_merge %>% 
  group_by(city, lat, lng, FIA_LANDUSE) %>% 
  summarize(n = n(), .groups = "drop") %>% 
  ggplot() +
  geom_col(aes(x = reorder(city, lng, decreasing = FALSE),
               y = n,
               fill = FIA_LANDUSE),
           position = "fill") +
  scale_fill_viridis_d(name = "Type", option = "H") +
  labs(x = "City (ordered by longitude)", y = "Proportion of city plots") +
  theme_bw(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
city_plots_merge %>% 
  ...
  summarize(n = n(), .groups = "drop") %>% 
  arrange(FIA_LANDUSE) %>% 
  pivot_wider(names_from = FIA_LANDUSE,
              values_from = n) %>% 
  arrange(lng) %>% 
  select(-lat, -lng) %>% 
  kable()
...
#### Land use type of plots with live trees

```{r land-use-plots-trees}
# Plot the proportion of city plots with trees by landuse type
trees_coords_merge %>% 
  group_by(city, lat, lng, FIA_LANDUSE) %>% 
  summarize(n = n_distinct(PLOTID)) %>% 
  ggplot() +
...
#### Land use type of live trees

```{r land-use-trees}
# Plot the proportion of city trees by landuse type
trees_coords_merge %>% 
  group_by(city, lat, lng, FIA_LANDUSE) %>% 
  summarize(n = n()) %>% 
  ggplot() +
...
#### Ecoregions I of cities

```{r ecoregions1, fig.width=11, fig.height=8.5}
...
tree_city_sf <- st_as_sf(tree_city_coords,
                         coords = c("lng", "lat"),
                         crs = st_crs(ecoregions1_simple))
...
# City ecoregions I table
city_ecoreg %>% 
  select(city, ecoregion1) %>% 
  kable()
```

#### Ecoregions II of cities

```{r ecoregions2, fig.width=11, fig.height=8.5}
# Simplify the polygons
ecoregions2_simple <- st_simplify(ecoregions2, preserveTopology = TRUE, dTolerance = 1000) %>% 
  st_transform(crs = "EPSG:4326")

# Turn coordinates into an sf object with correct CRS
tree_city_sf2 <- st_as_sf(tree_city_coords,
                          coords = c("lng", "lat"),
                         crs = st_crs(ecoregions2_simple))

# Plot the cities over ecoregions (level II)
ggplot() +
  geom_sf(data = ecoregions2_simple, aes(fill = NA_L2NAME)) +
  geom_sf(data = tree_city_sf2) +
  geom_sf_text(data = tree_city_sf2, aes(label = camelname),
               size = 2, angle = -30, nudge_x = 0.75, nudge_y = -0.75) +
  scale_fill_viridis_d(name = "Ecoregion II", option = "H") +
  theme_void(base_size = 8) +
  theme(legend.position = "bottom") +
  coord_sf(xlim = c(-125, -67), ylim = c(24, 49))

# City ecoregions II table
city_ecoreg %>% 
  select(city, ecoregion2) %>% 
  kable()
```

### City diversity {.tabset}
...
poolaccum_S %>% 
  ...
  theme_bw(base_size = 10) +
  theme(legend.position = "bottom")
ggsave("Results/species_accumulation_plots.pdf", height = 5, width = 7)
```
...
poolaccum_S_area %>% 
  ...
  theme_bw(base_size = 10) +
  theme(legend.position = "bottom")
ggsave("Results/species_accumulation_area.pdf", height = 5, width = 7)
```
...
#ttests
with(subset(div_own_tests, metric=='rarefied richness'), t.test(private, public, paired=T))#not sig
...
ggplot(data=filter(div_ownership, metric=='rarefied richness'), aes(x=MAP_mm, y=div, colour = ownership))+
  geom_point()+
  geom_smooth(method = 'lm')

c<-ggplot(data=filter(div_plant, metric=='rarefied richness'), aes(x=MAP_mm, y=div, colour = planted))+
  geom_point()+
  geom_smooth(method = 'lm')+
...
d<-ggplot(data=filter(div_plant, metric=='rarefied richness'), aes(x=Tmin_C, y=div, colour = planted))+
  geom_point()+
  geom_smooth(method = 'lm')+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = 'top')+
  ylab('Rarefied Richness')+
