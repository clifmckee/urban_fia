...
#### Land use type of city land aea
...
#### Number of live trees in forest by total land area

```{r land-area-forest-trees}
# Does the number of forest trees scale with total city size?
trees_coords_merge %>% 
  filter(FIA_LANDUSE %in% c("Forest land", "Rangeland/Chaparral")) %>% 
  group_by(city) %>% 
  summarize(n = n(), .groups = "drop") %>% 
  ...
  ggplot(aes(x = city_acres, y = n)) +
  geom_point() +
  labs(x = "City land area", y = "Natural land trees\n(forest, rangeland, and chaparral)") +
  theme_bw(base_size = 10)
